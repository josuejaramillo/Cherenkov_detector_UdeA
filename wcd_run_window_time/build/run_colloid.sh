...
for ((start=1; start<=total_iterations; start+=group_size)); do
  ...
  parallel 'echo "tiempo seed {}: $(time (./wcd_colloid -time 1 -output run_colloid -seed {} > ./parallel_outputs/colloid_output_{}.log 2>&1) 2>&1)" >> ./parallel_outputs/timing_colloid.txt' ::: $(seq $start $end)
done
