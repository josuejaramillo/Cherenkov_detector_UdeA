...
void G4HEPEvtInterface::GeneratePrimaryVertex(G4Event* evt)
{
  ...
  for( G4int IHEP=0; IHEP<NHEP; ++IHEP )
  {
    ...
    G4double PHEP6; // x position
	  G4double PHEP7; // y position
	  G4double PHEP8; // z position
	
    inputFile >> ISTHEP >> IDHEP >> JDAHEP1 >> JDAHEP2
              >> PHEP1 >> PHEP2 >> PHEP3 >> PHEP5 >> PHEP6 >> PHEP7 >> PHEP8;
    ...
    if(vLevel > 1)
    {
      // G4cout << "HEPEvntInterface:  " << " "<< ISTHEP << " "<< IDHEP<< " " << JDAHEP1 << " "<< JDAHEP2<< " "
      //         << PHEP1<< " " << PHEP2<< " " << PHEP3<< " " << PHEP5<< " " << PHEP6<< " " << PHEP7<< " " << PHEP8 << G4endl; //<< " " << ISTHEP << " " << IDHEP << " " << JDAHEP1
    //          << " " << JDAHEP2 << " " << PHEP1 << " " << PHEP2
    //          << " " << PHEP3 << " " << PHEP5 << " " << PHEP6 << " " << PHEP7 << " " << PHEP8 <<  G4endl;
    }
    ...
    //G4double p = sqrt(PHEP1*PHEP1 + PHEP2*PHEP2 + PHEP3*PHEP3);
    //G4ThreeVector momentumDirection(PHEP1/p, PHEP2/p, PHEP3/p);
    //particle->SetMomentumDirection(momentumDirection);
    
    //particle->SetParticlePosition(PHEP6*cm, PHEP7*cm, PHEP8*cm); //it does not work!!!
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////  
  ...
  vertex->SetPosition(PHEP6*cm, PHEP7*cm, PHEP8*cm);
