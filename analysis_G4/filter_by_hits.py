...
def load_root_data(files):
    ...
    for root_file in files:
        ...
        df_photon = ROOT.RDataFrame("photonData", file)
        df_muon = ROOT.RDataFrame("muonData", file)
        
        # Seleccionar las ramas y convertirlas en arrays de numpy
        wavelengths.extend(df_photon.AsNumpy(columns=["photonWavelength"])["photonWavelength"])
        energies.extend(df_photon.AsNumpy(columns=["photonEnergy"])["photonEnergy"])
        hits.extend(df_muon.AsNumpy(columns=["hits"])["hits"])
    return np.array(wavelengths), np.array(energies), np.array(hits)
    

# Cargar datos de ambos conjuntos de archivos
all_wavelengths_water, all_energies_water, hits_water = load_root_data(root_files_water)
all_wavelengths_colloid, all_energies_colloid, hits_colloid = load_root_data(root_files_colloid)

# Crear histogramas de longitudes de onda para ambos conjuntos de datos
counts_water, bin_edges = np.histogram(all_wavelengths_water[hits_water<1000], bins=70)
counts_colloid, _ = np.histogram(all_wavelengths_colloid[mask], bins=bin_edges)
...
plt.title("Photon Wavelength Histogram Comparison", fontsize=18)
plt.legend()
